...
class RB(nn.Module):
    def __init__(
        ...
        modules_body = []
        for i in range(2):
            modules_body.append(conv(n_feat, n_feat, kernel_size, bias=bias))  # default_conv
            if bn: modules_body.append(nn.BatchNorm2d(n_feat))
            if i == 0: modules_body.append(act)  # ReLU
...
class ResidualGroup(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, act, res_scale, n_resblocks=19):
...
## Residual Channel Attention Network (CRAN)
